...
def robot2_info(msg):
    global pos_robot2x, pos_robot2y, vel_robot2x, vel_robot2y, theta, robot2
...
while not ctrl_c:
    ...
    goal = Point()
    goal.x = robot2[0] + inc_x
    goal.y = robot2[1] + inc_y

    dist_goal = np.sqrt((goal.x - robot2[0]) ** 2 + (goal.y - robot2[1]) ** 2)
