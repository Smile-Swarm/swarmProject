...
def robot4_info(msg):
    global pos_robot4x, pos_robot4y, vel_robot4x, vel_robot4y, robot4
...
def robot5_info(msg):
    global pos_robot5x, pos_robot5y, vel_robot5x, vel_robot5y, theta, robot5
...
velocity = rospy.Publisher('/robot5/cmd_vel', geometry_msgs.msg.Twist,queue_size=1)
...
def compute_cohesion(nearest_agents):
    ...
    mean_position.x -= robot5[0]
    mean_position.y -= robot5[1]
...
def compute_separation(nearest_agents):
    ...
    direction = Vector2()
    for agent in nearest_agents:
        vec1.x = robot5[0] - agent[0]
        vec1.y = robot5[1] - agent[1]
...
def compute_migration():
    ...
    migration_point.x = leader[0] - robot5[0]
    migration_point.y = leader[1] - robot5[1]
...
def compute_avoid():
    ...
    for point in obstacle:
        vec1.x = robot5[0] - point[0]
        vec1.y = robot5[1] - point[1]
        distance = vec1.norm() #magnitude of vector = length
        vec1 = vec1 / distance #convert vec1 to unit vector
        vec1 = vec1 / distance #magnitude in opposite direction is inversely proportional to distance btw robots  
        direction += vec1
...
while not ctrl_c:
    ...
    goal = Point()
    goal.x = robot5[0] + inc_x
    goal.y = robot5[1] + inc_y

    dist_goal = np.sqrt((goal.x - robot5[0]) ** 2 + (goal.y - robot5[1]) ** 2)
